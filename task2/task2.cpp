...
bool check(const std::vector<unsigned>& vec) {
    auto range_vec = std::ranges::views::iota(0, 9) | 
        std::ranges::views::transform([&vec](const int i) { return vec[i]; });
...
int main() {
    ...
    for (auto i : std::ranges::views::iota(0, 9)) {
        auto col_range = std::ranges::views::iota(0, 9) | 
            std::ranges::views::transform([&matrix, &i](const int j) {
                    return matrix[i][j]; 
                    });
        auto quad_range = std::ranges::views::iota(0, 9) | 
            std::ranges::views::transform([&matrix, &i](const int j) { 
                    return matrix[i / 3 * 3 + j / 3][i % 3 * 3 + j % 3]; 
                    });
        if (!check(matrix[i]) || 
                !check({col_range.begin(), col_range.end()}) || 
                !check({quad_range.begin(), quad_range.end()})) {
