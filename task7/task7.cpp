...
void print_header(std::wostream& o) {
    o << "Group\t" << std::left << std::setw(40) << "Name" << 
        "Lb1\tLb2\tLb3\tLb4\tLb5\tLb6\tSum" << std::endl << std::right;
    o << std::setfill(L'-') << std::setw(100) << L'-' << std::setfill(L' ') << std::endl;
}

void print_debt_header(std::wostream& o) {
    o << "Group\t" << std::left << std::setw(40) << "Name" << 
        "Debt" << std::endl << std::right;
...
std::wstring print_student(const student& st) { 
    ...
    stream << st.group << L"\t";
    stream << std::setw(40) << std::left << 
        st.name[0] + L' ' + st.name[1] + L' ' + st.name[2] << std::right;
...
double calc_debt(const student& st) {
    if (st.sum >= 100) {
        if (std::all_of(st.grades.begin(), st.grades.end(), [](const double gr) { 
                    return gr >= 0; 
                    })) {
            return 0;
        }
        else {
            auto grrange = st.grades | 
                std::ranges::views::filter([](const double gr) { return gr < 0; });
...
int main() {
    ...
    for (std::wstring line; std::getline(input_file, line);) {
        ...
        linestream >> group;
        std::vector<std::wstring> name;
        std::copy_n(std::istream_iterator<std::wstring, wchar_t>(linestream), 3, 
                std::back_inserter(name));
        std::vector<double> grades;
        std::copy_n(std::istream_iterator<double, wchar_t>(linestream), 6, 
    ...
    std::ranges::copy(frange.begin(), frange.end(), 
    ...
    auto drange = students | 
        std::ranges::views::filter([&task2_group](const student& st) { 
                return st.group == task2_group; 
                }) | std::ranges::views::transform(print_student);
    std::wcout << L"Список студентов группы " << task2_group << std::endl << std::endl;
    print_header(std::wcout);
    std::ranges::copy(drange.begin(), drange.end(), 
            std::ostream_iterator<std::wstring, wchar_t>(std::wcout, L"\n"));

    //Task 7.3
    auto trange = students | std::ranges::views::filter([&task2_group](const student& st) { 
            return st.group == task2_group; 
            }) | 
    ...
    std::wcout << std::endl;
    std::wcout << L"Список студентов, имеющих право на досрочный зачет" << 
        std::endl << std::endl;
    print_header(std::wcout);
    std::ranges::copy(trange.begin(), trange.end(), 
            std::ostream_iterator<std::wstring, wchar_t>(std::wcout, L"\n"));
    
    //Task 7.4
    auto fourth_range = students | 
        std::ranges::views::filter([&task2_group](const student& st) { 
                return st.group == task2_group; }) | 
        std::ranges::views::transform([](const student& st) { 
            std::wstringstream stream;
            stream  << st.group << L"\t";
            stream << std::setw(40) << std::left << 
                st.name[0] + L' ' + st.name[1] + L' ' + st.name[2] << 
                calc_debt(st) << std::right;
            return stream.str();
            });
    std::wcout << L'\n';
    std::wcout << L"Сумма долга на зачет у студентов группы " << 
        task2_group << std::endl << std::endl;
    print_debt_header(std::wcout);
    std::ranges::copy(fourth_range.begin(), fourth_range.end(), 
            std::ostream_iterator<std::wstring, wchar_t>(std::wcout, L"\n"));
    ...
    std::wcin >> task5_name;
    std::wcout << L'\n';
    auto lower = std::ranges::lower_bound(students, task5_name, {}, 
            [](const student& st) { return st.name[0]; });
    auto upper = std::ranges::upper_bound(students, task5_name, {}, 
            [](const student& st) { return st.name[0]; });
    auto fifth_range = std::ranges::subrange(lower, upper) |
        std::ranges::views::transform(print_student);
    std::wcout << L"Поиск данных по фамилии" << std::endl << std::endl;
    print_header(std::wcout);
    std::ranges::copy(fifth_range.begin(), fifth_range.end(), 
            std::ostream_iterator<std::wstring, wchar_t>(std::wcout, L"\n"));
